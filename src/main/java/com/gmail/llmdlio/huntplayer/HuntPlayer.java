    ...
    public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("hunt")) {
			...
			if (isInvisible || target.getGameMode() == GameMode.SPECTATOR) {
				sender.sendMessage(
						String.valueOf(pluginPrefix) + args[0] + " cannot be found, they are invisible!");
				return true;
			}
			...
			if (usingTowny) {
				TownyHook.townyHook(target, sender);
			}
			...
			if (seeker.getLocation().getWorld() == target.getLocation().getWorld()) {
				...
				} else if (seekerHeight > targetHeight && seekerHeight - targetHeight >= 1) {
				...
				} else if (seekerHeight < targetHeight && targetHeight - seekerHeight >= 1) {
